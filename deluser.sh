...
# carregar o xray_lib.sh de forma transitiva. Como o source do
# atlas.sh acima tem erros suprimidos (2>/dev/null) e não é fatal se
# faltar, xray_remove_client_safe() podia não existir quando este
# script rodasse sozinho. Agora carregamos a lib explicitamente aqui
# também, igual ao addtest.sh já faz.
...
fun_limpar_user() {
    ...
    # 2. Remove do Xray
    # [CORREÇÃO] Antes este bloco editava o config.json direto via jq,
    # SEM flock — diferente do resto do projeto, que desde a correção
    # do bug "User X already exists" (ver xray_lib.sh) sempre usa
    # xray_add_client_safe()/xray_remove_client_safe() com lock. Isso
    # podia causar uma escrita perdida se o cron do Atlas (que grava
    # no mesmo config.json, com lock) rodasse no mesmíssimo instante
    # que alguém removesse um usuário por aqui — um clássico "last
    # write wins" silencioso. Agora usa a mesma função com lock que
    # todo o resto do projeto já usa, incluindo o kick do limit.sh.
    if type xray_remove_client_safe &>/dev/null; then
        xray_remove_client_safe "$user_to_del"
    elif [ -f "$XRAY_CONF" ]; then
        # Fallback só usado se xray_lib.sh não puder ser carregado por
        # algum motivo (não deveria acontecer numa instalação normal).
        # Mantido sem lock, igual ao comportamento original, só como
        # rede de segurança pra não deixar a remoção falhar por completo.
...
# -------------------------------------------------------
# Modo automático (chamado pelo addtest.sh via 'at', pelo menu.sh
# na opção "Excluir Expirados" e pelo atlas.sh quando detecta que um
# usuário sumiu do Atlas via sync de 1min)
# -------------------------------------------------------
if [[ "$2" == "--auto" ]]; then
    user="$1"
